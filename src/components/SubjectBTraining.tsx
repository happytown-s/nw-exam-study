...
const STORAGE_STATS = 'nw-b-stats';
const questions: Question[] = subjectBData as Question[];
...
export default function SubjectBTraining() {
  ...
  const nextQuestion = () => {
    if (idx + 1 >= pool.length) { setMode('menu'); return; }
    setIdx(idx + 1);
    setSelected(null);
    setAnswered(false);
    setShowExplanation(false);
  ...
  if (mode === 'quiz') {
    ...
    return (
      <div>
        ...
        <div className="text-xs mb-2 px-2 py-1 rounded inline-block" style={{ backgroundColor: '#1a1a3e', color: '#f39c12' }}>
          {currentQ.category}
        </div>
  ...
  return (
    <div>
      <div className="mb-2 p-3 rounded-lg text-center" style={{ backgroundColor: '#1a1a3e' }}>
        <div className="text-xs mb-1" style={{ color: '#94a3b8' }}>Subject B (25 questions, 105 min)</div>
        <div className="text-xs" style={{ color: '#f39c12' }}>Network Design, Protocol Analysis, Troubleshooting, Security, Cloud, Migration</div>
      </div>
      ...
      <div className="mb-4">
        ...
        <input type="range" min="5" max="25" step="5" value={count} onChange={e => setCount(Number(e.target.value))} className="w-full" />
      </div>
